...
const AvailableAppointments = ({ selectedDate }) => {
    ...
    const date = format(selectedDate, 'PP');  // module 74-5

    // using "React Query" [no need useEffect/useState]
    ...
    // useEffect(() => {
    //     fetch('https://doctors-portal-server-khaki-theta.vercel.app/appointmentOptions')
    //         .then(res => res.json())
    ...
    return (
        <section className='my-16'>
            <p className='font-bold text-center text-secondary'>Available Appointment on {format(selectedDate, 'PP')}</p>
